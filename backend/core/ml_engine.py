...
class EnsembleMLEngine:
    ...
    def evaluate_threat(self, processed_input: Dict[str, Any], heuristic_score: float, deep_analysis: bool = False) -> Tuple[bool, Optional[float], Dict[str, Any]]:
        ...
        try:
            ...
            # The confidence values from secbert_classifier use the sqrt arc formula,
            # deliberately calibrated so that avg confidence ~0.87 gives secbert_score ~8.7,
            # which feeds the 70% weight to produce a correct Critical score for full kill chains.
            secbert_score = 5.0
            if detected_ttps:
